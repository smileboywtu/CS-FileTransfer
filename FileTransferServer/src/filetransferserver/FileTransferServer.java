/*
 *      this is the server of the file transfer system
 *  here we will use the multi-thread to receive the file from the client
 *
 */
    ...
    public void acceptClient() throws IOException {
        ...
        while (true) {
            ...
            if (getServerMode()) {
                ...
                (new Thread(new FileStoreService(acceptSocket))).start();
                // notify user
...
class FileStoreService implements Runnable {
    ...
    private final MongoClient user = new MongoClient();

    // constructor
    public FileStoreService(Socket soc) throws IOException {
        // here just record the client
        client = soc;
        // set the stream
        in = new ObjectInputStream(client.getInputStream());
        // set the dir
        fileStoreDir = "./FileStoreDirectory/";
        // create directory
        if(!new File(fileStoreDir).exists()){
            Files.createDirectory(Paths.get(fileStoreDir));
        }
        // create the database
        sync = new UsingMongoSavaFile(user);
    }
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                Logger.getLogger(FileTransferServer.class.getName()).log(
